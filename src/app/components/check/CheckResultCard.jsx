...
export default function CheckResultCard() {
  ...
  useEffect(() => {
    async function updateSearchResults() {
      ...
      try {
        ...
        setSearchError(
          error.message ||
            "Live search is temporarily unavailable. Showing saved reports instead.",
        );
      }
  ...
  if (matchedReports.length === 0) {
    return (
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        {searchError && (
          <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-orange-200 bg-orange-50 p-5 text-sm font-semibold text-orange-800 sm:flex-row sm:items-center sm:justify-between">
            <p>{searchError} Saved reports may be incomplete.</p>
            <button
              type="button"
